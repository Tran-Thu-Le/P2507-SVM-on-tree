...
"""
Complete benchmark (FAIR) giữa:
- SVM On Tree (C++ core qua pybind11)
- sklearn LinearSVC (primal)

Eval: 2N = X + X_proj (projection lên spine).
- Tree prediction dùng cached threshold trên t_eval = [t_train; t_train] => O(2N)
- LinearSVC prediction => O(2N * d)

Fit time cho Tree có 3 chế độ (chọn bằng --tree-fit-mode):
- total: time_total (bao gồm A+B+C+D+E)
- no_sort: time_no_spine_sort (total - sort spine)
- dp: time_dp (chỉ DFS + DP, KHÔNG comparable trực tiếp với LinearSVC fit)

Dataset names hỗ trợ:
elongated, linear, banana, blobs, moons, circles, breast_cancer, wine, iris, all
"""
...
# # Datasets (2D, low-dim)
# # -------------------------
# def ds_linear(n_samples=2000, seed=0):
#     X, y = make_classification(
#         n_samples=n_samples, n_features=2, n_redundant=0, n_informative=2,
...
# def ds_elongated(n_samples=2000, seed=0):
#     rng = np.random.default_rng(seed)
#     n0 = n_samples // 2
#     n1 = n_samples - n0
#     # elongated along x (spine), with separation along x
#     mu0 = np.array([-3.0, 0.0])
#     mu1 = np.array([+3.0, 0.0])
#     cov0 = np.array([[4.0, 0.5], [0.5, 1.0]], dtype=np.float64)
#     cov1 = np.array([[4.0, -0.5], [-0.5, 1.0]], dtype=np.float64)
#     X0 = rng.multivariate_normal(mu0, cov0, n0)
#     X1 = rng.multivariate_normal(mu1, cov1, n1)
#     X = np.vstack([X0, X1]).astype(np.float64)
#     y = np.hstack([np.zeros(n0, np.int64), np.ones(n1, np.int64)])
#     return X, y

# def ds_banana(n_samples=2000, seed=0):
#     rng = np.random.default_rng(seed)
#     n0 = n_samples // 2
#     n1 = n_samples - n0
#     t0 = np.linspace(0, np.pi, n0)
#     t1 = np.linspace(0, np.pi, n1)
#     r0 = 3 + 0.5 * rng.standard_normal(n0)
#     r1 = 3 + 0.5 * rng.standard_normal(n1)
#     X0 = np.column_stack([r0*np.cos(t0),  r0*np.sin(t0) + 1.0])
#     X1 = np.column_stack([r1*np.cos(t1), -r1*np.sin(t1) - 1.0])
#     X = np.vstack([X0, X1]).astype(np.float64)
#     y = np.hstack([np.zeros(n0, np.int64), np.ones(n1, np.int64)])
#     return X, y

# def ds_blobs(n_samples=2000, seed=0):
#     X, y = make_blobs(
#         n_samples=n_samples, centers=2, n_features=2,
#         cluster_std=1.5, random_state=seed
#     )
#     return X.astype(np.float64), y.astype(np.int64)

# def ds_moons(n_samples=2000, seed=0):
#     X, y = make_moons(n_samples=n_samples, noise=0.12, random_state=seed)
#     return X.astype(np.float64), y.astype(np.int64)

# def ds_circles(n_samples=2000, seed=0):
#     X, y = make_circles(n_samples=n_samples, noise=0.12, factor=0.6, random_state=seed)
#     return X.astype(np.float64), y.astype(np.int64)

# def ds_breast_cancer(seed=0):
#     data = load_breast_cancer()
#     X = StandardScaler().fit_transform(data.data)
#     X2 = PCA(n_components=2, random_state=seed).fit_transform(X)
#     y = data.target.astype(np.int64)
#     return X2.astype(np.float64), y

# def ds_wine(seed=0):
#     data = load_wine()
#     X = StandardScaler().fit_transform(data.data)
#     y = (data.target == 0).astype(np.int64)  # binary
#     X2 = PCA(n_components=2, random_state=seed).fit_transform(X)
#     return X2.astype(np.float64), y
...
#     # use 2 best features
#     X2 = X[:, [2, 3]].astype(np.float64)
#     return X2, y


# def gen_dataset(name: str, n_samples: int, seed: int):
#     name = name.lower()
#     if name == "elongated":
#         return ds_elongated(n_samples, seed)
#     if name == "linear":
#         return ds_linear(n_samples, seed)
#     if name == "banana":
#         return ds_banana(n_samples, seed)
#     if name == "blobs":
#         return ds_blobs(n_samples, seed)
#     if name == "moons":
#         return ds_moons(n_samples, seed)
#     if name == "circles":
#         return ds_circles(n_samples, seed)
#     if name == "breast_cancer":
#         return ds_breast_cancer(seed)
#     if name == "wine":
#         return ds_wine(seed)
#     if name == "iris":
#         return ds_iris(seed)
...
#     w = (m1 - m0).astype(np.float64)
#     nrm = np.linalg.norm(w)
#     w_unit = w / (nrm + 1e-12) if nrm > 0 else np.ones_like(w) / np.sqrt(len(w))
...
#     if ts <= tp:
#         y_left, y_right = int(y_train[s_idx]), int(y_train[p_idx])
#     else:
#         y_left, y_right = int(y_train[p_idx]), int(y_train[s_idx])

#     return dict(t_train=t_train, thr=float(thr), y_left=y_left, y_right=y_right)

# def predict_tree_2n_cached(params):
...
# # One-run evaluators
# # -------------------------
# def run_tree_once(X, y, lam, fit_mode: str):
#     out = fit_core(X, y, lam)
#     s_idx = int(out["support_s"])
#     p_idx = int(out["support_p"])

#     if fit_mode == "total":
#         t_fit = float(out["time_total"])
#         fit_label = "total"
#     elif fit_mode == "no_sort":
#         t_fit = float(out["time_no_spine_sort"])
#         fit_label = "no_sort"
#     elif fit_mode == "dp":
#         t_fit = float(out["time_dp"])
#         fit_label = "dp"
#     else:
#         raise ValueError("fit_mode must be one of: total, no_sort, dp")

#     params = precompute_tree_params(X, y, s_idx, p_idx)

#     y_eval = np.hstack([y, y])
#     t1 = time.perf_counter()
#     y_pred = predict_tree_2n_cached(params)
#     t_pred = time.perf_counter() - t1
...
# # Benchmark driver
# # -------------------------
# def bench_one_dataset(name, n_samples, repeats, lam, seed, standardize, warmup, fit_mode):
#     X, y = gen_dataset(name, n_samples, seed)

#     if standardize:
#         X = StandardScaler().fit_transform(X).astype(np.float64)

#     print(f"\n{'='*70}")
#     print(f"DATASET: {name} | N={X.shape[0]} d={X.shape[1]} | standardize={standardize}")
#     print(f"Eval = 2N (X + X_proj), repeats={repeats}, lamda={lam}, tree_fit_mode={fit_mode}")
#     print(f"{'='*70}")

#     if warmup:
#         _ = run_tree_once(X, y, lam, fit_mode)
#         _ = run_linearsvc_once(X, y)

#     R_tree = [run_tree_once(X, y, lam, fit_mode) for _ in range(repeats)]
...
#     print(f"FIT   Tree({fit_mode}): {fit_tree:.6f}s   vs   LinearSVC: {fit_svc:.6f}s   | Speedup FIT: {speedup_fit:.2f}x")
#     print(f"PRED  Tree(cached):     {pred_tree:.6f}s   vs   LinearSVC: {pred_svc:.6f}s   | Speedup PRED: {speedup_pred:.2f}x")
#     print(f"ACC   Tree:             {acc_tree:.4f}     vs   LinearSVC: {acc_svc:.4f}")
#     print(f"Support=(s={s_idx}, p={p_idx}), Loss={loss:.6f}")
...
#     parser.add_argument("--tree-fit-mode", type=str, default="total",
#                         choices=["total","no_sort","dp"],
#                         help="Tree fit timing mode: total | no_sort | dp")
#     args = parser.parse_args()

#     print("=== Complete Benchmark: SVM On Tree (C++) vs sklearn LinearSVC (primal) on 2N eval ===")
#     print(f"standardize={args.standardize}, repeats={args.repeats}, lamda={args.lamda}, seed={args.seed}, warmup={not args.no_warmup}")
#     print("Tip fairness threads: OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1")

#     datasets = ["elongated","linear","breast_cancer","wine","iris","blobs","moons","banana","circles"] if args.dataset=="all" else [args.dataset]
#     for ds in datasets:
#         bench_one_dataset(
#             ds, args.n_samples, args.repeats, args.lamda, args.seed,
#             args.standardize, warmup=(not args.no_warmup), fit_mode=args.tree_fit_mode
#         )
...
# -------------------------
# Tree prediction (FAIR): compute dot-products on X_eval
# -------------------------
def predict_tree_e2e(model_dict, X_eval):
    m0 = np.asarray(model_dict["m0"], dtype=np.float64)
    w  = np.asarray(model_dict["w"],  dtype=np.float64)
    thr = float(model_dict["thr"])
    yl = int(model_dict["y_left"])
    yr = int(model_dict["y_right"])
...
# -------------------------
# One-run evaluators (FAIR wall-clock)
# -------------------------
def run_tree_once(X, y, lam):
    # FIT: wall-clock Python time around fit_core
    t0 = time.perf_counter()
    out = fit_core(X, y, lam)
    t_fit = time.perf_counter() - t0
    ...
    # PRED: wall-clock, includes dot-products on X_eval (fair vs LinearSVC.predict)
    t1 = time.perf_counter()
    y_pred = predict_tree_e2e(out, X_eval)
    t_pred = time.perf_counter() - t1
    ...
    return {
        ...
        "pred_s": float(t_pred),
        "acc": float(acc),
        "support": (int(out["support_s"]), int(out["support_p"])),
        "loss": float(out["min_val"]),
...
# Benchmark driver
# -------------------------
def bench_one_dataset(name, n_samples, repeats, lam, seed, standardize, warmup):
    X, y = gen_dataset(name, n_samples, seed)
    ...
    print(f"\n{'='*70}")
    print(f"DATASET: {name} | N={X.shape[0]} d={X.shape[1]} | standardize={standardize}")
    print(f"Eval = 2N (X + X_proj), repeats={repeats}, lamda={lam}")
    print(f"{'='*70}")
...
def main():
    ...
    print("=== FAIR Benchmark: SVM On Tree (C++/pybind) vs sklearn LinearSVC (primal) on 2N eval ===")
    print(f"standardize={args.standardize}, repeats={args.repeats}, lamda={args.lamda}, seed={args.seed}, warmup={not args.no_warmup}")
    print("Threads pinned: OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1")

    datasets = ["elongated","linear","breast_cancer","wine","iris","blobs","moons","banana","circles"] if args.dataset=="all" else [args.dataset]
    for ds in datasets:
        bench_one_dataset(
            ds, args.n_samples, args.repeats, args.lamda, args.seed,
            args.standardize, warmup=(not args.no_warmup)
        )
