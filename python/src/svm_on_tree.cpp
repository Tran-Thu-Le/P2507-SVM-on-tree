...
//         }
//     }
//     // post-order: sort by depth desc (phần của DP)
//     order_post = order_pre;
//     std::sort(order_post.begin(), order_post.end(),
//               [&](int a, int b){ return depth[a] > depth[b]; });
// }
...
//     // ===== Stage A: means/projections (no timing cut) =====
//     auto tA0 = Clock::now();
//     std::vector<double> m0(D, 0.0), m1(D, 0.0);
//     long long c0 = 0, c1 = 0;
//     for (int64_t i=0;i<N;i++){
...
//     // ===== Stage B: sort along spine (cần loại khỏi fit-time theo yêu cầu) =====
//     auto tB0 = Clock::now();
//     std::vector<int> idx(N); for (int i=0;i<N;i++) idx[i]=i;
//     std::sort(idx.begin(), idx.end(), [&](int a, int b){ return t[a] < t[b]; });
...
//     // ===== Stage D: DP/DFS passes (bao gồm root_tree_iter và các accumulate) =====
//     auto tD0 = Clock::now();
//     auto compute_side = [&](int root_node, int target_node, std::vector<double>& out_maxtr){
//         std::vector<int> parent; std::vector<double> parent_w; std::vector<int> depth; std::vector<int> order_post;
//         root_tree_iter(G, root_node, parent, parent_w, depth, order_post);
...
//     out["time_sort_spine"]   = time_sort_spine;      // <-- phần cần loại khỏi “fit” theo yêu cầu
//     out["time_build_graph"]  = time_build_graph;
//     out["time_dp"]           = time_dp;
//     out["time_scan_pairs"]   = time_scan_pairs;
//     out["time_no_spine_sort"]= time_no_spine_sort;   // tiện dùng trực tiếp
...
//           R"pbdoc(
//               Core routine for SVM On Tree (timed stages):
//               Inputs:
//                 X: (N, d) float64
//                 y: (N,) int64 with labels 0/1
//                 lambda_: float
...
//                  time_total, time_means_proj, time_sort_spine,
//                  time_build_graph, time_dp, time_scan_pairs, time_no_spine_sort}
//           )pbdoc");
// }
...
static void root_tree_iter(
    ...
    // Per-node iterator offset over adjacency in CSR
    ...
    while (!stack.empty()){
        ...
        if (it[u] < (R - L)) {
            ...
            parent_w[v] = G.weights[k];
            depth[v] = depth[u] + 1;
            stack.push_back(v);                 // dive deeper
        } else {
            // all children processed -> emit u in post-order
                  ...
                  double lambda_)
{
    ...
    // ===== Stage D: DP/DFS passes (rooting + accumulations) =====
    auto tD0 = Clock::now();
    ...
    out["time_build_graph"]   = time_build_graph;
    out["time_dp"]            = time_dp;            // <<— DFS + DP only (đã tối ưu)
    out["time_scan_pairs"]    = time_scan_pairs;
...
PYBIND11_MODULE(svm_on_tree_cpp, m) {
    m.doc() = "C++ core for SVM On Tree (pybind11, timed stages)";
    m.def("fit_core", &fit_core, py::arg("X"), py::arg("y"), py::arg("lambda_")=1.0,
          R"pbdoc(
              Core routine for SVM On Tree (timed stages):
              Inputs:
    ...
    // Tag để kiểm tra đúng bản build
    m.attr("BUILD_TAG") = "root_postorder_no_sort_v1";
}
