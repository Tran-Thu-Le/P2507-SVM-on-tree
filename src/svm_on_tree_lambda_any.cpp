...
// src/svm_on_tree_lambda_any.cpp
#include <pybind11/pybind11.h>
#include <pybind11/numpy.h>
#include <pybind11/stl.h>
...
// Compute directed sums along the root->target spine-path for each node on that path.
// IMPORTANT FIX (fair+correct): dis0/dis1 must accumulate FULL subtrees (including leaves).
// Only out_maxtr[u] is restricted to the child that lies on the root->target path.
static void accumulate_noise_on_branch(
    const CSR& G,
    int root, int target,
    const std::vector<int>& order_post,
    ...
    for (int u : order_post) {
        for (long long k = G.indptr[u]; k < G.indptr[u+1]; ++k) {
            ...
            // ALWAYS accumulate full subtree (includes leaf children)
            dis0[u] += dis0[v] + (double)sz0[v] * w;
            dis1[u] += dis1[v] + (double)sz1[v] * w;

            // Only this part is restricted to the path-to-target child
            if (!contains[v]) continue;
...
// Helpers: build numpy arrays (copy) for returning model params
static py::array_t<double> vec_to_np1(const std::vector<double>& v) {
    py::array_t<double> arr((py::ssize_t)v.size()); // 1D
    auto a = arr.mutable_unchecked<1>();
    for (py::ssize_t i=0;i<(py::ssize_t)v.size();++i) a(i) = v[(size_t)i];
    return arr;
}
                  ...
                  double lambda_)
{
    ...
    // spine node ids in sorted order (projection nodes)
    std::vector<int> spine_ids(N);
    for (int k=0;k<(int)N;k++) spine_ids[(size_t)k] = (int)N + idx[(size_t)k];

    // pos map for spine nodes
    std::vector<int> pos_of_node(TOT, -1);
    for (int k=0;k<(int)N;k++) pos_of_node[(size_t)spine_ids[(size_t)k]] = k;
    ...
    // "to_right": toward increasing spine position
    // "to_left" : toward decreasing spine position
    if (X0_sorted.size()>=2){
        compute_side(X0_sorted.front(), X0_sorted.back(),  maxtr0_to_right);
        compute_side(X0_sorted.back(),  X0_sorted.front(), maxtr0_to_left);
    } else if (X0_sorted.size()==1){
    ...
    if (is_unit) {
        // O(n): scan adjacent spine edges (between consecutive projection nodes)
        // evaluate BOTH orders for completeness.
        for (int k=0;k<(int)N-1;k++){
            int u = spine_ids[(size_t)k];
            int v = spine_ids[(size_t)k+1];
            if (colors[(size_t)u]==colors[(size_t)v]) continue;

            double d = Dsp[(size_t)k+1] - Dsp[(size_t)k]; // adjacent spine distance
        ...
        // ============================================================
        // λ != 1: REQUIRED behavior: scan ALL nodes (leaf + spine) => 2N nodes
        // Still O(n^2) (more precisely O((2N)^2)).
        // We extend sum_to_right/left from spine to leaf using:
        //   dist(leaf_i, X) = dist(spine_i, X) + Lleaf[i]
        // so sums add + Lleaf[i] * (#nodes in that side, same label).
        // ============================================================

        // pos_node: spine position for ANY node (leaf uses its spine partner)
        auto pos_node = [&](int node)->int{
            if (node >= (int)N) return pos_of_node[(size_t)node];                  // spine node
            else                return pos_of_node[(size_t)((int)N + node)];       // leaf -> its projection node
        };
        auto off_node = [&](int node)->double{
            return (node < (int)N) ? Lleaf[(size_t)node] : 0.0;
        };

        // prefix counts over spine positions by label (positions correspond to original points)
        std::vector<int> pref0((int)N+1, 0), pref1((int)N+1, 0);
        for (int k=0;k<(int)N;k++){
            int oi = idx[(size_t)k];           // original index at position k
            int c  = (int)Yptr[oi];
        ...
        // counts of SAME-LABEL NODES (not positions) to right/left in augmented tree
        auto cnt_right_nodes_same_label = [&](int node)->long long{
            ...
            return 2LL * (long long)pos_right; // each position contributes (leaf+spine) => 2 nodes
        };
        ...
        // full tree distance between any nodes
        ...
        // scan all node pairs (u < v), and evaluate the ordered pair (left-position, right-position)
        for (int u=0; u<TOT; ++u){
            for (int v=u+1; v<TOT; ++v){
                if (colors[(size_t)u] == colors[(size_t)v]) continue;

                int pu = pos_node(u), pv = pos_node(v);
                // (leaf & its spine partner share position, but they also share label => already filtered)
                if (pu == pv) continue;
    ...
    // Convert best_u/best_v (tree node id) -> original data index in [0..N-1]
    int su = (best_u >= (int)N) ? (best_u - (int)N) : best_u;
    int sv = (best_v >= (int)N) ? (best_v - (int)N) : best_v;
    ...
    // (optional) debug: which node-types were picked
    // out["best_u_node"] = best_u;
    // out["best_v_node"] = best_v;
...
PYBIND11_MODULE(svm_on_tree_cpp, m) {
    m.doc() = "SVM On Tree C++ core (pybind11). Fair-ready: returns model params for E2E prediction.";
    m.def("fit_core", &fit_core, py::arg("X"), py::arg("y"), py::arg("lambda_")=1.0);
    m.attr("BUILD_TAG") = "fair_v2_any_lambda_all_nodes_n2";
}
