...
\begin{align}
d(x,y) &: \text{distance between nodes } x \text{ and } y \text{ in the tree}, \\
...
\[
\mathrm{subtree\_cost}(p_2,p_1) = d(p_4,p_2) + d(p_3,p_2).
\]
...
f(s,p_2) &= \mathrm{subtree\_cost}(p_2,p_1) - d(p_2,p_3),
\end{align}
where $n'(p_1,s)$ represents the number of vertices with the same label as $p_1$ in the relevant subtree.

Therefore:
\[
f(s,p_1) = f(s,p_2) + n'(p_1,s) \cdot y + d(p_2,p_3).
\]

Since $d(s,p_1) = d(s,p_2) + y$, the loss functions are:
\begin{align}
L(s,p_1) &= f(s,p_1) - d(s,p_1), \\
L(s,p_2) &= f(s,p_2) - d(s,p_2).
\end{align}

Substituting the expressions:
\[
L(s,p_1) = L(s,p_2) + n'(p_1,s) \cdot y + d(p_2,p_3) - y.
\]

Since $n'(p_1,s) \cdot y \geq 0$, $d(p_2,p_3) \geq 0$, and typically $n'(p_1,s) \geq 1$, we have:
\[
L(s,p_1) \geq L(s,p_2).
\]
...
\subsection{Detailed derivation of {$f(s,p1)$ and $f(s,p2)$}}

Define $tol(x,y)$ as the sum of distances from $x$ to all descendant nodes with the same label as $x$, when the tree is rooted at $y$.

We assume without loss of generality that $s \in V_+$ and $p_1, p_2\in V_-$ and $p_2$ is in the path connecting $s$ and $p_1$, \ie $d(s, p_1) = d(s, p_2) + d(p_2, p_1)$. 
...
\begin{equation*}
    f(s, p_1) - f(s, p_1) = f_-(p_1, s) - f_-(p_2, s).
\end{equation*}
...
The optimal support pair $(s,p)$ must consist of two adjacent vertices on the spine \remLE{prove that "on spine" not "on spoke".}
