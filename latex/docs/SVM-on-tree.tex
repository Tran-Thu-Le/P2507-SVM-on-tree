...
\end{proposition}
\begin{proof}
    We use dynamic programming to efficiently compute these quantities.

    \textbf{Step 1:} Sort the projected points $x'_1, x'_2, \ldots, x'_n$ in order along the spine.

    \textbf{Step 2:} For each relation $\sim \in \{<, =, >, \leq, \geq\}$, we compute using recurrence relations:

    For relation "$<$": $d^{< i}_+ = d^{< (i-1)}_+ + \mathbf{1}_{y_{i-1}=+1} \cdot [d(x'_{i-1}, x'_i) + d(x_{i-1}, x'_i)]$

    For relation "$>$": $d^{> i}_+ = d^{> (i+1)}_+ + \mathbf{1}_{y_{i+1}=+1} \cdot [d(x'_{i+1}, x'_i) + d(x_{i+1}, x'_i)]$

    For relation "$=$": $d^{= i}_\pm = d(x_i, x'_i)$ if $y_i = \pm 1$, otherwise $0$

    For relations "$\leq$" and "$\geq$": $d^{\leq i}_\pm = d^{< i}_\pm + d^{= i}_\pm$

    \textbf{Step 3:} Each update takes $O(1)$ time. Total complexity: $O(5n) = O(n)$.
\end{proof}
