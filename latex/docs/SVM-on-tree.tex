...
\end{proposition}
\begin{proof}
    We prove this by showing that all quantities can be computed using dynamic programming on the tree structure.

    Consider the tree where each data point $x_i$ is connected to its projection $x'_i$ on the spine. The key insight is that for each index $i$, the quantities $d^{\sim i}_\pm$ and $N^{\sim i}_\pm$ can be computed by analyzing the subtree structure around node $i$.

    For relation "$<$": The quantities $d^{< i}_\pm$ and $N^{< i}_\pm$ represent the accumulated distances and counts from all nodes $j$ with $j < i$. These can be computed by a single traversal of the left subtree rooted at $i$, where:
    $$d^{< i}_\pm = \sum_{j < i, y_j = \pm 1} [d(x'_j, x'_i) + d(x_j, x'_i)]$$
    $$N^{< i}_\pm = 2|\{j < i: y_j = \pm 1\}|$$

    For relation "$>$": Similarly, $d^{> i}_\pm$ and $N^{> i}_\pm$ are computed by traversing the right subtree.

    For relation "$=$": We have $d^{= i}_\pm = d(x_i, x'_i)$ if $y_i = \pm 1$, otherwise $0$, and $N^{= i}_\pm = 2$ if $y_i = \pm 1$, otherwise $0$.

    For relations "$\leq$" and "$\geq$": These are computed as $d^{\leq i}_\pm = d^{< i}_\pm + d^{= i}_\pm$ and $N^{\leq i}_\pm = N^{< i}_\pm + N^{= i}_\pm$, and similarly for "$\geq$".
