...
\end{proposition}
\begin{proof}
    We use DFS-based dynamic programming on tree to efficiently compute these quantities.

    \textbf{Algorithm:} For each index $i$, we perform DFS traversal from node $i$ as root:

    \textbf{Step 1:} \texttt{dfs\_sz()} computes subtree sizes:
    For each subtree rooted at $v$, compute $sz1[v] = |\{j \in subtree(v): y_j = +1\}|$ and $sz2[v] = |\{j \in subtree(v): y_j = -1\}|$

    \textbf{Step 2:} \texttt{dfs()} computes accumulated distances:
    For each node $v$, compute $dis\_s[v] = \sum_{j \in subtree(v), y_j=+1} d(x_j, x'_j)$ and $dis\_p[v] = \sum_{j \in subtree(v), y_j=-1} d(x_j, x'_j)$

    \textbf{Step 3:} Extract required quantities:
    $N^{< i}_+ = 2 \cdot sz1[left\_subtree(i)]$, $N^{< i}_- = 2 \cdot sz2[left\_subtree(i)]$
    $N^{> i}_+ = 2 \cdot sz1[right\_subtree(i)]$, $N^{> i}_- = 2 \cdot sz2[right\_subtree(i)]$
    $d^{< i}_+ = dis\_s[left\_subtree(i)]$, $d^{< i}_- = dis\_p[left\_subtree(i)]$
    $d^{> i}_+ = dis\_s[right\_subtree(i)]$, $d^{> i}_- = dis\_p[right\_subtree(i)]$

    Relations $\leq$ and $\geq$ are computed by combining $<$, $=$, and $>$ results.

    \textbf{Complexity:} Each DFS takes $O(n)$ time. We perform DFS for each of the $n$ nodes, but by carefully organizing the computation and using the tree structure, we can compute all 20 quantities for all indices in total $O(n)$ time through a single traversal with appropriate bookkeeping.

    This matches the implementation in \texttt{compute\_loss()} function which efficiently computes $sz1, sz2, dis\_s, dis\_p$ arrays in one DFS pass.
\end{proof}
