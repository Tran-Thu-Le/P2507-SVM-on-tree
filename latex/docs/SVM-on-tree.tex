...
For each index $i$, let us define $20$ quantities corresponding to five relations $\sim$ in $\{<, =, >, \leq, \geq\}$ \remLE{need meaning of them}
\begin{align*}
    d^{\sim i}_\pm & = \sum_{j \sim i, y_j=\pm 1} d(x'_j, x'_i) + d(x_j, x'_i),\\
    N^{\sim i}_\pm & = 2 |\{j \in \{1,..., n\}: j \sim i \text{ and } y_j=\pm 1\}|
\end{align*}

These quantities can be computed in linear time using dynamic programming.
\begin{proposition}
    The $20$ quantities for all index $i=1,..., n$ can be computed in $O(n)$.
\end{proposition}
\begin{proof}
    We prove this by showing that all quantities can be computed using a single DFS traversal on the tree.

    Consider the tree structure where each data point $x_i$ is connected to its projection $x'_i$ on the spine. For any node $v$ in the tree, let $subtree(v)$ denote the subtree rooted at $v$. We define:
    $$sz_+(v) = |\{j \in subtree(v): y_j = +1\}|, \quad sz_-(v) = |\{j \in subtree(v): y_j = -1\}|$$
    $$dist_+(v) = \sum_{j \in subtree(v), y_j=+1} d(x_j, x'_j), \quad dist_-(v) = \sum_{j \in subtree(v), y_j=-1} d(x_j, x'_j)$$

    These quantities satisfy the recurrence relations:
    $$sz_\pm(v) = \mathbf{1}_{y_v = \pm 1} + \sum_{u \in children(v)} sz_\pm(u)$$
    $$dist_\pm(v) = \mathbf{1}_{y_v = \pm 1} \cdot d(x_v, x'_v) + \sum_{u \in children(v)} [dist_\pm(u) + sz_\pm(u) \cdot weight(v,u)]$$

    Now, for each index $i$ corresponding to projection $x'_i$ on the spine, the quantities $d^{\sim i}_\pm$ and $N^{\sim i}_\pm$ can be extracted directly from the DFS computation:
    $$N^{< i}_\pm = 2 \cdot sz_\pm(left\_subtree(i)), \quad N^{> i}_\pm = 2 \cdot sz_\pm(right\_subtree(i))$$
    $$d^{< i}_\pm = dist_\pm(left\_subtree(i)), \quad d^{> i}_\pm = dist_\pm(right\_subtree(i))$$
    $$N^{= i}_\pm = 2\mathbf{1}_{y_i = \pm 1}, \quad d^{= i}_\pm = \mathbf{1}_{y_i = \pm 1} \cdot d(x_i, x'_i)$$

    For relations $\leq$ and $\geq$, we have $d^{\leq i}_\pm = d^{< i}_\pm + d^{= i}_\pm$ and $N^{\leq i}_\pm = N^{< i}_\pm + N^{= i}_\pm$, and similarly for $\geq$.

    Since the DFS traversal computes $sz_\pm$ and $dist_\pm$ for all nodes in $O(n)$ time, and extracting all 20 quantities for all indices requires only $O(1)$ operations per index, the total complexity is $O(n)$.\end{proof}
...
\begin{proof}
    ...
    It is well known that $d(u, v)$ will be computed in $O(n^2)$. If $f(u, v)$ can also be computed in $O(n^2)$, then we know all $L_\lambda(u, v)$. In the remaining of the proof, we focus on the cumputation of $f(u, v)$.


    It is well known that $d(u, v)$ can be computed in $O(n^2)$ using standard tree algorithms. For $f(u, v)$, we use the 20 precomputed quantities from Proposition 1.

    Given support nodes $u$ and $v$, the noise function is:
    $f(u, v) = \sum_{j \in V_-(u,v)} [d(x_j, x'_j) + d(x'_j, u) + d(x'_j, v)]$

    Let $i_u$ and $i_v$ be the indices corresponding to positions of $u$ and $v$ on the spine. Using our precomputed quantities:
    $f(u, v) = \sum_{\sim \in \{<,=,>\}} [d^{\sim i_u}_- + d^{\sim i_v}_-]$ where the summation is taken over appropriate ranges.

    Since each $f(u,v)$ computation uses precomputed values in $O(1)$ time, and there are $O(n^2)$ pairs, the total complexity is $O(n^2)$.

    Therefore, $L_\lambda(u,v) = f(u,v) - \lambda d(u,v)$ can be computed in $O(n^2)$ for all pairs.
\end{proof}
...
Recall $x'_1, \ldots,x'_n$ are projections of $x_1,\ldots, x_n$ and we assume that $x'_1, \ldots,x'_n$ have been sorted in a certain order, \eg by their first coordinates. By the adjacency property (Theorem~\ref{thm:adjacency}), we know that support vectors form a pair $(x'_i, x'_{i+1})$ with opposite signs. By previous section, we know that $f(x'_i, x'_{i+1})$ and, thus, $L_1(x'_i, x'_{i+1})$ can be precomputed in $O(n)$.
