...
\subsection{Tree-based approximation}
\label{sec:tree-based-approximation}
Given a training dataset $\{(x_i,y_i)\in \mathbb{R}^d \times \{-1,+1\}: i=1,..., n\}$, we describe a procedure for constructing a tree that approximates the dataset. Let $\Delta$ be a line in $\mathbb{R}^d$. In this work, we focus on the line $\Delta$ connecting the two class means. For each $i=1,\ldots,n$, let $x'_i$ denote the orthogonal projection of $x_i$ onto $\Delta$. The tree is then built with vertices consisting of both the original points $x_i$ and their projections $x'_i$. Its edge set includes (i) spline edges, which are the line segments connecting consecutive projection points along $\Delta$, and (ii) spoke edges, which are the line segments connecting each original point $x_i$ to its projection $x'_i$. The details of this construction are described below.
...
and edge set
\begin{equation*}
E = {(x'i, x'{i+1}): i=1,..., n-1} \cup {(x_i, x'_i): i=1,..., n}.
\end{equation*}
...
Let $T$ be a tree constructed in \Cref{sec:tree-based-approximation}.
For two vertices $u,v$ in $T$ with different labels, we define the noise terms:
\begin{align}
...
\subsection{Adjacency property for unit parameter}
